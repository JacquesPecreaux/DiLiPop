function [Cxm,Cym,su,old_su]=...
            virtual_time_loop_LS_Chan_Vese_Band_curvature_flat_noi(lambdaInside,area_diff_stop,max_iter,small_step_limit,display_every_n,energy_every_n,...
          ...
          % we use in this case in lagrangian band base energy, corrected
        % from slope in z, which mean that we have a peak over space
        % distances.
...
%             [Lap4,rns]=Curvature_LS(Level_set_fct, param.LS_dx,param.LS_dy, param.LS_alpha,param.LS_algo_diff, 1,lambda*kappa);
%         else
%             Der=zeros(size(Level_set_fct),14);
%         end
        ImageeDer=Derivatives_LS(Imagee, param.(param_set).LS_dx,param.(param_set).LS_dy, param.(param_set).LS_alpha,param.(param_set).LS_algo_diff, 1,1,1);
...
%         %% energies
%     [Ekappa,Esigma,Edl,Ein,Eout,Econ]=mathematica_generated_LS_Band_Energy_flat_curv_noi_debug(Der,Level_set_fct,lambda,kappa,sigma_used,extensionmax,...
...
% if 0
% %% evolution
%     tmp_in=mathematica_generated_LS_Band_flat_curv_noi(Der,Level_set_fct,0,kappa,sigma_used,extensionmax,...
...
%     
%     tmp_out=mathematica_generated_LS_Band_flat_curv_noi(Der,Level_set_fct,0,kappa,sigma_used,extensionmax,...
...
%     
%     tmp_con=mathematica_generated_LS_Band_flat_curv_noi(Der,Level_set_fct,0,kappa,sigma_used,extensionmax,...
...
%     
%     tmp_curv=mathematica_generated_LS_Band_flat_curv_noi(Der,Level_set_fct,lambda,kappa,0,extensionmax,...
%         0,0,0,c1_,c2_,c3_,Imagee,ImageeDer,param.epsilon,param.Bandwidth,dl0m);
...
%     title('Curvature');
%     
%     tmp_sig=mathematica_generated_LS_Band_flat_curv_noi(Der,Level_set_fct,lambda,0,sigma_used,extensionmax,...
%         0,0,0,c1_,c2_,c3_,Imagee,ImageeDer,param.epsilon,param.Bandwidth,dl0m);
...
% %%
% end
new_step_=mathematica_generated_LS_Band_flat_curv_noi(Der,Level_set_fct,lambda,kappa,sigma_used,extensionmax,...
