...
result3 = result1+result2+(c3.^2.*lambdaContour+(-2).*c3.*Imagee.*lambdaContour+Imagee.^2.*( ...
  lambdaContour+(-1).*lambdaOutside)+(-1).*c2.^2.*lambdaOutside+2.* ...
  c2.*Imagee.*lambdaOutside).*DiracDeltaApprox(LevelSetFct,epsilon)+ ...
  (-1).*BandWidth.*lambdaInside.*(Der(:,:,1).^2+Der(:,:,2).^2).^(-1/2).*(( ...
  -1).*Der(:,:,1).^2+(-1).*Der(:,:,2).^2+((Der(:,:,1).^2+Der(:,:,2).^2).*( ...
  Der(:,:,3).^2+Der(:,:,5).^2)).^(1/2)).*DiracDeltaApprox(LevelSetFct, ...
  epsilon).^2+BandWidth.*lambdaInside.*(Der(:,:,1).^2+Der(:,:,2).^2).^( ...
  -3/2).*((-2).*Der(:,:,1).*Der(:,:,4).*Der(:,:,2)+Der(:,:,3).*Der(:,:,2).^2+ ...
  Der(:,:,1).^2.*Der(:,:,5)).*DiracDeltaApprox(LevelSetFct,epsilon).* ...
